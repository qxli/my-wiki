...
loginmanager.login_view = 'site.user_login'
...
class Processors(object):
    ...
    def wikilink(self, html):
        ...
        for i in compLink.findall(html):
            ...
            formattedLink = u"<a href='{0}'>{1}</a>".format(url_for('site.display', url=url), title)
    ...
    def out(self):
        ...
        # md = markdown.Markdown(markdown_extensions)
        # html = md.convert(self.content)
...
idx = Index(app.config.get('WHOOSH_BASE'))

from views import site
app.register_blueprint(site, url_prefix='/wiki')
